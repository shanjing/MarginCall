...
# MarginCall end-user UI lifecycle manager.
# Controls the custom frontend served by `server.py` (not ADK Dev UI).
#
# Commands:
#   start    Start end-user UI in background
#   stop     Stop end-user UI
#   restart  Restart end-user UI
#   status   Show runtime/model/session status
#
# Options:
#   -p <port>   UI port (default: 8080)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
VENV_ACTIVATE="${PROJECT_ROOT}/.venv/bin/activate"
RUNTIME_DIR="${PROJECT_ROOT}/.tmp"
PID_FILE="${RUNTIME_DIR}/margincall-ui.pid"
LOG_FILE="${RUNTIME_DIR}/margincall-ui.log"
PORT=8080
COMMAND=""

usage() {
  cat <<EOF
Usage: $(basename "$0") [-p PORT] {start|stop|restart|status}
...
start_ui() {
  ...
  if collect_ui_pids >/dev/null && [[ -n "$(collect_ui_pids || true)" ]]; then
    log "End-user UI appears to be running; stopping existing process first."
    stop_ui
  fi

  log "Starting MarginCall end-user UI on http://127.0.0.1:${PORT}"
  (
    cd "${PROJECT_ROOT}"
  ...
  for ((i = 1; i <= retries; i++)); do
    if curl -fsS "http://127.0.0.1:${PORT}/" >/dev/null 2>&1; then
      log "MarginCall UI started (pid ${pid}). Log: ${LOG_FILE}"
      open_ui_in_browser
      return 0
    fi
    sleep 1
...
stop_ui() {
  local base_pids
  base_pids="$(collect_ui_pids || true)"

  if [[ -z "${base_pids}" ]]; then
    log "No running MarginCall end-user UI process found."
    rm -f "${PID_FILE}"
    return 0
  fi
...
EOF
  ...
  log "Stopping MarginCall UI process tree: ${all_pids[*]}"
  kill -TERM "${all_pids[@]}" 2>/dev/null || true

  if ! wait_for_exit 10 "${all_pids[@]}"; then
    log "Graceful stop timed out; force-killing remaining processes."
    kill -KILL "${all_pids[@]}" 2>/dev/null || true
  fi

  rm -f "${PID_FILE}"
  log "MarginCall UI stopped."
}

status_ui() {
  local pids ai_model db_path ui_running
  pids="$(collect_ui_pids || true)"
  ...
  if [[ -n "${pids}" ]]; then
    ui_running=1
    log "MarginCall end-user UI is running on port ${PORT}."
    printf '%s\n' "${pids}" | awk '{print "  pid: "$1}'
    if [[ -f "${PID_FILE}" ]]; then
      log "PID file: ${PID_FILE} ($(tr -d '[:space:]' < "${PID_FILE}"))"
    fi
  else
    log "MarginCall end-user UI is not running on port ${PORT}."
    if [[ -f "${PID_FILE}" ]]; then
